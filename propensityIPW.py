...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 22 06:23:31 2022
...
for line in open(filename):
    ...
    for word in set(tokens[1:]):
        if word not in vocab:
            # the order in which a word appears in the document (starting 0)
...
for treatment in sorted(vocab):  # each word is a treatment
    rows = []
    cols = []
    values = []
    y = []
    ...
    for line in open(filename):
        ...
        # y records treatment/control status 1 for treated; 0 for control
        y.append(contains_treatment)
        l += 1
        if l > 200:
            break

    data = coo_matrix((values, (rows, cols)), shape=(
        l, V)).toarray()  # data is the sparse matrix
    ...
    #prop_scores_arr = np.array(prop_scores)
    weight_arr = np.array([1/p for p in prop_scores])

    model.fit(data, y, weight_arr)
    new_scores = model.predict_proba(data)
    # propSD = np.std(new_scores)
    ...
    # pvalue
    chiscores[treatment] = float(new_scores[treatment])
